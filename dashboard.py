...
def load_data():
    ...
    # ➕ Parse city from US-style addresses
    df["City"] = df["Purchase Address"].str.extract(r",\s*([^,]+),\s*[A-Z]{2}\s*\d+")
    df["Country"] = "USA"  # Hardcoded — update logic if dataset includes other countries
